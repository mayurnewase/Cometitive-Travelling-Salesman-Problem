...
class ClassOpt:
	...
	def connectToEnviroment(self , host , port):
	...
	def findNearestNeighbour(self , currentCityId , visited):
		...
		dist = self.df.loc[currentCityId,"dist0":"dist5"]			#fetch distance values for current city
	...
	def findCostOfTravel(self , route):
		...
		for i in range(len(route) - 1):
			row_data = self.df.loc[route[i] , "dist0" : "dist5"]
